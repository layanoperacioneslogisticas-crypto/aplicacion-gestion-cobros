...
function resolvePdfUrl(row) {
  return String((row && (row.pdf_url || row.boleta_firmada_url || row.firma_boleta_url)) || '').trim();
}

function validateStageRequirements(targetIdx, row) {
  const pdf = resolvePdfUrl(row);
...
cobrosRouter.get('/', async (req, res) => {
  ...
  res.json({
    success: true,
    rows: (data || []).map((row) => Object.assign({}, row, { pdf_url: resolvePdfUrl(row) })),
    total: count || 0,
    limit,
    offset
  });
...
cobrosRouter.get('/:id', async (req, res) => {
  ...
  res.json({ success: true, cobro: Object.assign({}, cobro, { pdf_url: resolvePdfUrl(cobro) }), items: items || [] });
});
