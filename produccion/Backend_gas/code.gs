...
function generarPDFEstiloNuevo(id, datos, items, targetFolder) {
    ...
    const signPilotoUrl = escapeHtml_(datos.firmaPiloto || '');
    const signVistaUrl = escapeHtml_(datos.firmaVista || '');
    const signPiloto = signPilotoUrl
        ? `<img src="${signPilotoUrl}" style="${imgStyle}">`
        : `<div class="sig-empty">Sin firma</div>`;
    const signVista = signVistaUrl
        ? `<img src="${signVistaUrl}" style="${imgStyle}">`
        : `<div class="sig-empty">Sin firma</div>`;
    ...
    const html = buildCobroPdfHtml_({
        id: escapeHtml_(id),
        fechaStr: escapeHtml_(fechaStr),
        horaStr: escapeHtml_(horaStr),
        ...
        factura: factura,
        totalCajas: String(totalCajas),
        totalLineas: String(totalLineas),
        totalUnidadesSueltas: String(totalUnidadesSueltas),
        totalMonto: Number(totalMonto).toFixed(2),
        rows: rows,
        signVista: signVista,
        signPiloto: signPiloto
    });
...
function buildDashboardStats_(actorEmail) {
    ...
    if (shA && shA.getLastRow() > 1) {
        ...
        for (let i = 0; i < rows.length; i++) {
            ...
            const monto = rowMontoCobro_(r, map);
    ...
    while (endRow > 1 && out.length < lim) {
        ...
        for (let i = data.length - 1; i >= 0 && out.length < lim; i--) {
            ...
            out.push({
                ...
                c9: r[7] || '',
                factura: r[8] || '',
                total: rowMontoCobro_(r, map),
                estado: normalizeEstado_(r[COL_ESTADO - 1]),
                licencia: r[23] || '',
                pdfUrl: resolveCobroPdfUrl_(r, map)
            });
        }
        endRow = startRow - 1;
    }
...
function adminDeleteUser(row, actorEmail) {
    ...
    if (!sh) return { success: false, message: "Hoja 'Usuarios' no encontrada." };
    if (rowNum < 2 || rowNum > sh.getLastRow()) {
        return { success: false, message: 'Fila de usuario invalida.' };
    }

    const headers = sh.getRange(1, 1, 1, Math.max(sh.getLastColumn(), USER_SHEET_HEADERS.length)).getValues()[0] || [];
    const meta = buildUserHeaderMeta_(headers);
    const rowValues = sh.getRange(rowNum, 1, 1, headers.length).getValues()[0] || [];
    const user = mapUserRow_(rowValues, rowNum, meta);
    if (!actorCanAccessCatalogCountry_(actorCtx.profile, user.countryCode)) {
        return { success: false, message: 'No tiene acceso a este usuario fuera de su entorno.' };
    }

    const association = getUserDeleteAssociation_(user);
...
function adminDeletePilot(row, actorEmail) {
    ...
    const association = getPilotDeleteAssociation_(mapPilotoRow_(rowValues, rowNum, meta));
    if (association) return association;
...
function adminDeleteProvider(row, actorEmail) {
    ...
    const association = getProviderDeleteAssociation_(mapProveedorRow_(rowValues, rowNum, meta));
    if (association) return association;
...
function adminDeleteMaestroItem(row, actorEmail) {
    ...
    if (rowNum <= 0 && requestedCode) {
        ...
        for (let i = 1; i < all.length; i++) {
            const existingCode = String(maestroCell_(all[i], meta, 'codigo', 0) || '').trim().toLowerCase();
            const existingCountry = resolveCatalogCountryCode_(maestroCell_(all[i], meta, 'countryCode', 7));
        ...
        if (sameCountryMatches.length === 1) {
            ...
            return {
                success: false,
                message: 'Se encontraron multiples items con el mismo codigo en este pais. Revise los duplicados antes de eliminar.'
            };
        } else if (codeOnlyMatches.length > 0) {
            return {
                success: false,
                message: 'El codigo existe en otro pais. Revise el entorno del item antes de eliminarlo.'
            };
        }
    }

    if (rowNum < 2 || rowNum > sh.getLastRow()) {
        return { success: false, message: 'No se pudo identificar el item maestro a eliminar.' };
    }
    ...
    const association = getMaestroDeleteAssociation_(mapMaestroRow_(rowValues, rowNum, meta));
    if (association) return association;
...
function getCfgMailTransport_() {
    ...
    const from = String(getConfigValue_('sesFrom') || getConfigValue_('resendFrom') || props.getProperty(RESEND_FROM_PROP) || '').trim();
    const replyTo = String(getConfigValue_('sesReplyTo') || getConfigValue_('resendReplyTo') || props.getProperty(RESEND_REPLY_TO_PROP) || '').trim();
    const region = String(getConfigValue_('sesRegion') || 'us-east-1').trim() || 'us-east-1';
    const enabledFlag = boolConfig_('sesEnabled', boolConfig_('resendEnabled', false));
    const fallbackToMailApp = boolConfig_('sesFallbackMailApp', boolConfig_('resendFallbackMailApp', true));
    const autoEnabled = Boolean(from && region);
    const effective = getResendMailConfig_();
    return {
        sesEnabled: enabledFlag || autoEnabled,
        sesEnabledManual: enabledFlag,
        sesFallbackMailApp: fallbackToMailApp,
        sesRegion: region,
        sesFrom: from,
        sesReplyTo: replyTo,
        ...
        effectiveProvider: effective.enabled ? 'ses' : 'mailapp'
    };
}
...
function saveMailTransportConfig(mailObj, actorEmail) {
    ...
    const props = PropertiesService.getScriptProperties();
    const enabled = parseBoolLoose_(mailObj && (mailObj.sesEnabled != null ? mailObj.sesEnabled : mailObj.resendEnabled), false);
    const fallbackToMailApp = parseBoolLoose_(mailObj && (mailObj.sesFallbackMailApp != null ? mailObj.sesFallbackMailApp : mailObj.resendFallbackMailApp), true);
    const from = String(mailObj && (mailObj.sesFrom || mailObj.resendFrom || '')).trim();
    const replyTo = csvEmails_(mailObj && (mailObj.sesReplyTo || mailObj.resendReplyTo || ''));
    const region = String(mailObj && (mailObj.sesRegion || mailObj.resendApiUrl || 'us-east-1')).trim() || 'us-east-1';
    ...
    return {
        success: true,
        mailTransport: getCfgMailTransport_(),
        message: 'Configuración de Amazon SES actualizada.'
    };
}
...
function resumenIncidencia_(itemsJson) {
    try {
        ...
        const summary = summarizeIncidenciasFromValues_(arr.map(it => it && it.incidencia));
...
function getOptionalSheetGrid_(sheetName, aliases) {
    const ss = getDataStore_();
    const names = [sheetName].concat(aliases || []);
    let sh = ss.getSheetByName(sheetName);
    if (!sh) sh = findSheetByNormalizedName_(ss, names);
    if (!sh || sh.getLastRow() < 2) {
        return { sh: sh || null, headers: [], map: {}, rows: [] };
    }
    const lastCol = Math.max(1, sh.getLastColumn());
    return {
        sh: sh,
        headers: sh.getRange(1, 1, 1, lastCol).getValues()[0] || [],
        map: buildHeaderMap_(sh.getRange(1, 1, 1, lastCol).getValues()[0] || []),
        rows: sh.getRange(2, 1, sh.getLastRow() - 1, lastCol).getValues()
    };
}

function sameCatalogValue_(left, right) {
    const a = normalizeKey_(left);
    const b = normalizeKey_(right);
    return !!a && !!b && a === b;
}

function sameCatalogCountry_(catalogCountry, rowCountry) {
    const expected = normalizeCountryCode_(catalogCountry);
    const current = normalizeCountryCode_(rowCountry);
    if (!expected || !current) return true;
    return expected === current;
}

function pushAssociatedCobroId_(target, id) {
    const clean = String(id || '').trim();
    if (!clean) return;
    if (target.indexOf(clean) >= 0) return;
    target.push(clean);
}

function formatAssociatedCobroIds_(ids) {
    const list = Array.isArray(ids) ? ids.filter(Boolean) : [];
    if (!list.length) return '';
    const sample = list.slice(0, 3);
    const extra = list.length - sample.length;
    return extra > 0 ? (sample.join(', ') + ' y ' + extra + ' más') : sample.join(', ');
}

function buildCatalogAssociationResult_(entityLabel, displayValue, ids, extraMessage) {
    const refs = Array.isArray(ids) ? ids.filter(Boolean) : [];
    const quoted = String(displayValue || '').trim() ? (' "' + String(displayValue || '').trim() + '"') : '';
    let message = 'No se puede eliminar ' + entityLabel + quoted + ' porque está asociado a registros de boletas o del flujo de cobro.';
    if (refs.length) {
        message += ' Boletas relacionadas: ' + formatAssociatedCobroIds_(refs) + '.';
    }
    if (extraMessage) {
        message += ' ' + String(extraMessage).trim();
    }
    return {
        success: false,
        associated: true,
        linkedCobros: refs.slice(),
        message: message
    };
}

function buildCobroCountryIndex_() {
    const grid = getOptionalSheetGrid_('Aprobaciones');
    const out = {};
    for (let i = 0; i < grid.rows.length; i++) {
        const row = grid.rows[i] || [];
        const id = rowText_(row, grid.map, ['ID', 'Id']) || String(row[0] || '').trim();
        if (!id) continue;
        out[id] = normalizeCountryCode_(rowText_(row, grid.map, [WF.countryCode, 'CountryCode', 'Pais', 'PaisCodigo']));
    }
    return out;
}

function parseItemsJsonArray_(raw) {
    if (!raw) return [];
    let value = raw;
    try {
...
function userMatchesAssociationValue_(user, rawValue) {
    const value = String(rawValue || '').trim();
    if (!value) return false;
    const email = String(user && user.email || '').trim().toLowerCase();
    const nombre = String(user && user.nombre || '').trim();
    if (email && value.toLowerCase() === email) return true;
    if (email && sameCatalogValue_(value, email)) return true;
    if (nombre && sameCatalogValue_(value, nombre)) return true;
    return false;
}

function getProviderDeleteAssociation_(provider) {
    const grid = getOptionalSheetGrid_('Aprobaciones');
    const refs = [];
    const data = provider || {};
    for (let i = 0; i < grid.rows.length; i++) {
        const row = grid.rows[i] || [];
        const cobroId = rowText_(row, grid.map, ['ID', 'Id']) || String(row[0] || '').trim();
        const rowCountry = rowText_(row, grid.map, [WF.countryCode, 'CountryCode', 'Pais', 'PaisCodigo']);
        if (!sameCatalogCountry_(data.countryCode, rowCountry)) continue;
        const providerCode = rowText_(row, grid.map, ['ProveedorCodigo', 'CodigoProveedor', 'Proveedor_Codigo', 'CodProveedor']) || String(row[3] || '').trim();
        const providerName = rowText_(row, grid.map, ['Proveedor', 'ProveedorNombre', 'NombreProveedor']) || String(row[2] || '').trim();
        if ((data.codigo && sameCatalogValue_(data.codigo, providerCode)) || (data.nombre && sameCatalogValue_(data.nombre, providerName))) {
            pushAssociatedCobroId_(refs, cobroId);
        }
    }
    return refs.length ? buildCatalogAssociationResult_('el proveedor', data.nombre || data.codigo, refs) : null;
}

function getPilotDeleteAssociation_(pilot) {
    const grid = getOptionalSheetGrid_('Aprobaciones');
    const refs = [];
    const data = pilot || {};
    for (let i = 0; i < grid.rows.length; i++) {
        ...
        const rowCountry = rowText_(row, grid.map, [WF.countryCode, 'CountryCode', 'Pais', 'PaisCodigo']);
        if (!sameCatalogCountry_(data.countryCode, rowCountry)) continue;
        const pilotName = rowText_(row, grid.map, ['Piloto', 'PilotoNombre', 'Chofer', 'Conductor']) || String(row[15] || '').trim();
        if (data.nombre && sameCatalogValue_(data.nombre, pilotName)) {
            pushAssociatedCobroId_(refs, cobroId);
        }
    }
    return refs.length ? buildCatalogAssociationResult_('el conductor', data.nombre || data.dni, refs) : null;
}

function getMaestroDeleteAssociation_(item) {
    const data = item || {};
    const cobroCountries = buildCobroCountryIndex_();
    const detailGrid = getOptionalSheetGrid_('Detalle_Cobros');
    const refs = [];

    for (let i = 0; i < detailGrid.rows.length; i++) {
        const row = detailGrid.rows[i] || [];
        const cobroId = rowText_(row, detailGrid.map, ['IDCobro', 'ID_Cobro', 'CobroId']) || String(row[0] || '').trim();
        if (!sameCatalogCountry_(data.countryCode, cobroCountries[cobroId])) continue;
        const itemCode = rowText_(row, detailGrid.map, ['Codigo', 'Codigo_Item', 'CodigoItem']) || String(row[1] || '').trim();
        const itemDesc = rowText_(row, detailGrid.map, ['Descripcion', 'Descripción', 'Detalle']) || String(row[2] || '').trim();
        if ((data.codigo && sameCatalogValue_(data.codigo, itemCode)) || (data.descripcion && sameCatalogValue_(data.descripcion, itemDesc))) {
            pushAssociatedCobroId_(refs, cobroId);
        }
    }

    if (!refs.length) {
        const cobroGrid = getOptionalSheetGrid_('Aprobaciones');
        for (let j = 0; j < cobroGrid.rows.length; j++) {
            const row = cobroGrid.rows[j] || [];
            const cobroId = rowText_(row, cobroGrid.map, ['ID', 'Id']) || String(row[0] || '').trim();
            const rowCountry = rowText_(row, cobroGrid.map, [WF.countryCode, 'CountryCode', 'Pais', 'PaisCodigo']);
            if (!sameCatalogCountry_(data.countryCode, rowCountry)) continue;
            const items = parseItemsJsonArray_(row[16]);
            const matched = items.some(function (entry) {
                const code = String(entry && (entry.codigo || entry.Codigo) || '').trim();
                const desc = String(entry && (entry.descripcion || entry.Descripcion || entry.detalle || entry.Detalle) || '').trim();
                return (data.codigo && sameCatalogValue_(data.codigo, code)) || (data.descripcion && sameCatalogValue_(data.descripcion, desc));
            });
            if (matched) pushAssociatedCobroId_(refs, cobroId);
        }
    }

    return refs.length ? buildCatalogAssociationResult_('el producto', data.descripcion || data.codigo, refs) : null;
}

function getUserDeleteAssociation_(user) {
    const data = user || {};
    const refs = [];
    const cobroCountries = buildCobroCountryIndex_();
    const cobroGrid = getOptionalSheetGrid_('Aprobaciones');

    for (let i = 0; i < cobroGrid.rows.length; i++) {
        const row = cobroGrid.rows[i] || [];
        const cobroId = rowText_(row, cobroGrid.map, ['ID', 'Id']) || String(row[0] || '').trim();
        const rowCountry = rowText_(row, cobroGrid.map, [WF.countryCode, 'CountryCode', 'Pais', 'PaisCodigo']);
        if (!sameCatalogCountry_(data.countryCode, rowCountry)) continue;
        const responsable = rowText_(row, cobroGrid.map, ['Responsable']) || String(row[14] || '').trim();
        const responsableEmail = resolveResponsableEmailFromRow_(row, cobroGrid.map);
        if (userMatchesAssociationValue_(data, responsable) || userMatchesAssociationValue_(data, responsableEmail)) {
            pushAssociatedCobroId_(refs, cobroId);
        }
    }

    const bitacoraGrid = getOptionalSheetGrid_(SHEET_BITACORA, ['Bitacora']);
    for (let j = 0; j < bitacoraGrid.rows.length; j++) {
        const row = bitacoraGrid.rows[j] || [];
        const cobroId = rowText_(row, bitacoraGrid.map, ['ID', 'CobroId']) || String(row[1] || '').trim();
        if (!sameCatalogCountry_(data.countryCode, cobroCountries[cobroId])) continue;
        const usuario = rowText_(row, bitacoraGrid.map, ['Usuario']) || String(row[2] || '').trim();
        if (userMatchesAssociationValue_(data, usuario)) {
            pushAssociatedCobroId_(refs, cobroId || 'BITACORA');
        }
    }

    const notifGrid = getOptionalSheetGrid_('Notificaciones');
    for (let k = 0; k < notifGrid.rows.length; k++) {
        const row = notifGrid.rows[k] || [];
        const cobroId = rowText_(row, notifGrid.map, ['CobroId', 'IDCobro']) || String(row[3] || '').trim();
        if (!sameCatalogCountry_(data.countryCode, cobroCountries[cobroId])) continue;
        const userEmail = rowText_(row, notifGrid.map, ['UserEmail', 'Email']) || String(row[2] || '').trim();
        if (userMatchesAssociationValue_(data, userEmail)) {
            pushAssociatedCobroId_(refs, cobroId || 'NOTIFICACION');
        }
    }

    const criticalGrid = getOptionalSheetGrid_(SHEET_APROBACIONES_CRITICAS, ['Aprobaciones_Criticas']);
    for (let m = 0; m < criticalGrid.rows.length; m++) {
        const row = criticalGrid.rows[m] || [];
        const cobroId = rowText_(row, criticalGrid.map, ['IDCobro', 'CobroId']) || String(row[3] || '').trim();
        if (!sameCatalogCountry_(data.countryCode, cobroCountries[cobroId])) continue;
        const requestedBy = rowText_(row, criticalGrid.map, ['SolicitadoPor']) || String(row[4] || '').trim();
        const approvedBy = rowText_(row, criticalGrid.map, ['AprobadoPor']) || String(row[8] || '').trim();
        if (userMatchesAssociationValue_(data, requestedBy) || userMatchesAssociationValue_(data, approvedBy)) {
            pushAssociatedCobroId_(refs, cobroId || 'APROBACION');
        }
    }

    return refs.length
        ? buildCatalogAssociationResult_('el usuario', data.nombre || data.email, refs, 'Quite primero su relación con la boleta o con el flujo.')
        : null;
}

let cobroDetalleSubtotalMapMemo_ = null;
let cobroDetalleIncidenciaMapMemo_ = null;

function rowNumber_(row, headerMap, names, fallback) {
    const fallbackNum = Number(fallback || 0);
    const value = rowVal_(row, headerMap || {}, names);
    if (value == null || value === '') return isNaN(fallbackNum) ? 0 : fallbackNum;
    if (typeof value === 'number') return isNaN(value) ? (isNaN(fallbackNum) ? 0 : fallbackNum) : value;
    const text = String(value)
        .replace(/\s/g, '')
        .replace(/^s\//i, '')
        .replace(/,/g, '')
        .trim();
    if (!text) return isNaN(fallbackNum) ? 0 : fallbackNum;
    const num = Number(text);
    return isNaN(num) ? (isNaN(fallbackNum) ? 0 : fallbackNum) : num;
}

function invalidateCobroDetalleSubtotalMap_() {
    cobroDetalleSubtotalMapMemo_ = null;
}

function invalidateCobroDetalleIncidenciaMap_() {
    cobroDetalleIncidenciaMapMemo_ = null;
}

function buildCobroDetalleSubtotalMap_() {
    const out = {};
    try {
        const ss = getDataStore_();
        ...
        for (let i = 0; i < rows.length; i++) {
            const row = rows[i];
            const cobroId = rowText_(row, map, ['IDCobro', 'ID_Cobro', 'CobroId']) || String(row[0] || '').trim();
            if (!cobroId) continue;

            const cantidad = rowNumber_(row, map, ['Cantidad'], row[3]);
            const precio = rowNumber_(row, map, ['Precio', 'Precio_Unitario', 'PrecioUnitario'], row[4]);
            const subtotal = rowNumber_(row, map, ['Subtotal', 'Total', 'Importe'], Number(cantidad || 0) * Number(precio || 0));
            out[cobroId] = Number(out[cobroId] || 0) + Number(subtotal || 0);
        }
    } catch (e) {
        return {};
    }
...
function getCobroDetalleSubtotal_(idCobro) {
    const id = String(idCobro || '').trim();
    if (!id) return 0;
    if (!cobroDetalleSubtotalMapMemo_) {
        cobroDetalleSubtotalMapMemo_ = buildCobroDetalleSubtotalMap_();
    }
    return Number(cobroDetalleSubtotalMapMemo_[id] || 0);
}

function rowMontoCobro_(row, headerMap) {
    const legacyFallback = row && row.length > 12 ? row[12] : 0;
    const direct = rowNumber_(row, headerMap, ['TotalCobro', 'Monto', 'Total', 'Monto_Cobro'], legacyFallback);
    if (direct > 0) return direct;

    const id = rowText_(row, headerMap || {}, ['ID', 'Id']) || String(row && row[0] || '').trim();
    const detailSubtotal = getCobroDetalleSubtotal_(id);
    return detailSubtotal > 0 ? detailSubtotal : direct;
}

function normalizeIncidenciaValue_(value) {
    const raw = String(value == null ? '' : value).trim();
    if (!raw) return '';
    if (raw === 'DaÃ±ado') return 'Dañado';

    const key = normalizeKey_(raw);
    if (key === 'conforme') return 'Conforme';
    if (key === 'faltante') return 'Faltante';
    if (key === 'cruzado') return 'Cruzado';
    if (key === normalizeKey_('Dañado') || key === normalizeKey_('DaÃ±ado')) return 'Dañado';
    return raw;
}

function summarizeIncidenciasFromValues_(values) {
    const list = Array.isArray(values) ? values : [];
    const mapPlural = { 'Faltante': 'Faltantes', 'Dañado': 'Dañados', 'Cruzado': 'Cruzados', 'Conforme': 'Conforme' };
    const set = {};
    let hasExplicit = false;

    for (let i = 0; i < list.length; i++) {
        const inc = normalizeIncidenciaValue_(list[i]);
        if (!inc) continue;
        hasExplicit = true;
        if (inc !== 'Conforme') set[inc] = true;
    }

    const keys = Object.keys(set);
    if (keys.length) {
        return {
            value: keys.map(k => mapPlural[k] || k).join(' / '),
            hasExplicit: true
        };
    }
    return {
        value: hasExplicit ? 'Conforme' : '',
        hasExplicit: hasExplicit
    };
}

function buildCobroDetalleIncidenciaMap_() {
    const out = {};
    try {
        const ss = getDataStore_();
        const sh = ss.getSheetByName('Detalle_Cobros');
        ...
        for (let i = 0; i < rows.length; i++) {
            ...
            out[cobroId].hasRows = true;
            const incidencia = rowVal_(row, map, ['Incidencia', 'Observacion', 'Observación', 'Motivo']);
            const legacyFallback = (incidencia == null || incidencia === '') ? row[6] : incidencia;
            out[cobroId].incidencias.push(legacyFallback);
        }
    } catch (e) {
        return {};
    }
    ...
    for (let j = 0; j < ids.length; j++) {
        const id = ids[j];
        const entry = out[id];
        const summary = summarizeIncidenciasFromValues_(entry.incidencias);
        summaryMap[id] = {
            hasRows: !!entry.hasRows,
            hasExplicit: !!summary.hasExplicit,
...
function getCobroDetalleIncidenciaInfo_(idCobro) {
    const id = String(idCobro || '').trim();
    if (!id) {
        return {
            hasRows: false,
    ...
    if (!cobroDetalleIncidenciaMapMemo_) {
        cobroDetalleIncidenciaMapMemo_ = buildCobroDetalleIncidenciaMap_();
    }
    return cobroDetalleIncidenciaMapMemo_[id] || {
        hasRows: false,
        hasExplicit: false,
        value: ''
    };
...
function resolveCobroIncidencia_(idCobro, itemsJson) {
    const fromItems = resumenIncidencia_(itemsJson);
    if (fromItems) return fromItems;

    const detailInfo = getCobroDetalleIncidenciaInfo_(idCobro);
    if (detailInfo.value) return detailInfo.value;
    if (detailInfo.hasRows && !detailInfo.hasExplicit) return 'Sin dato migrado';
    return 'Conforme';
}
...
function validaRequisitosEntrada_(toIdx, row, headerMap) {
    const pdf = resolveCobroPdfUrl_(row, headerMap);
...
function resolveCobroPdfUrl_(row, headerMap) {
    const pdfUrl = String(row && row[COL_PDF_URL - 1] || '').trim();
    if (pdfUrl) return pdfUrl;
    return rowText_(row, headerMap || {}, [WF.boletaFirmadaUrl, WF.firmaBoletaUrl]);
}
...
function sendObservedReturnNotification_(ctx) {
    ...
    const detalle = {
        ...
        monto: Number(data.monto != null ? data.monto : rowMontoCobro_(row, headerMap)),
        pdfUrl: String(data.pdfUrl || resolveCobroPdfUrl_(row, headerMap)).trim(),
...
function getResendMailConfig_() {
    ...
    const replyTo = String(getConfigValue_('sesReplyTo') || getConfigValue_('resendReplyTo') || props.getProperty(RESEND_REPLY_TO_PROP) || '').trim();
    const region = String(getConfigValue_('sesRegion') || 'us-east-1').trim() || 'us-east-1';
    const enabledByCfg = boolConfig_('sesEnabled', boolConfig_('resendEnabled', false));
    const fallbackToMailApp = boolConfig_('sesFallbackMailApp', boolConfig_('resendFallbackMailApp', true));
    const autoEnabled = Boolean(from && region);
    return {
        enabled: Boolean((enabledByCfg || autoEnabled) && from && region),
...
function sendWithResend_(toCsv, subject, body, ccCsv, cfg) {
    ...
    if (!toArr.length) return { success: false, provider: 'ses', message: 'Sin destinatario.' };
    try {
        ...
        if (ccArr.length) payload.cc = ccArr.join(',');
        const rtArr = splitEmails_(cfg.replyTo);
        if (rtArr.length === 1) payload.replyTo = rtArr[0];
        if (rtArr.length > 1) payload.replyTo = rtArr.join(',');
        MailApp.sendEmail(payload);
        return { success: true, provider: 'ses', statusCode: 200 };
    } catch (e) {
        return { success: false, provider: 'ses', message: String(e) };
    }
}
...
function testResendSend(destino) {
    ...
    if (!cfg.enabled) {
        const resDisabled = { success: false, message: 'Amazon SES no configurado o deshabilitado.', config: cfgOut };
        logResultadoConsola_('[testResendSend]', resDisabled);
        return resDisabled;
    }
    const subject = '[TEST] Amazon SES Cobro Transporte';
    const body = 'Prueba de envio desde la capa legacy a traves de Amazon SES.\nFecha: ' + formatDateTimeSafe_(new Date());
    const res = sendWithResend_(toCsv, subject, body, '', cfg);
    const out = {
        success: Boolean(res && res.success),
        provider: String(res && res.provider || 'ses'),
...
/**
 * Valida la configuracion base de Amazon SES en la capa legacy.
 */
function validarConexionResend() {
    ...
    if (!cfg.from) {
        const resNoFrom = {
            success: false,
            connected: false,
            message: 'Falta configurar el remitente SES.',
            config: cfgOut
        };
        logResultadoConsola_('[validarConexionResend]', resNoFrom);
        return resNoFrom;
    }
    const resOk = {
        success: true,
        connected: true,
        statusCode: 200,
        probeUrl: 'SES bridge (Railway)',
        message: 'Configuracion SES lista. Use "Enviar prueba" para validar el flujo completo en Railway.',
        config: cfgOut
    };
...
function onEtapaChanged_(ctx) {
    ...
    const detalle = {
        ...
        monto: rowMontoCobro_(row, headerMap),
        pdfUrl: resolveCobroPdfUrl_(row, headerMap),
    ...
    const notifyRecipients = notifyEnabled
        ...
        : { to: '', cc: '' };
    const proveedorEmail = resolveProveedorEmail_(row, headerMap, detalle.proveedorCodigo, detalle.proveedor);
    ...
    if (idxEtapa === 2 && notifyEnabled) {
        const to = proveedorEmail || notifyRecipients.to;
        const cc = proveedorEmail ? mergeEmails_(notifyRecipients.to, notifyRecipients.cc) : notifyRecipients.cc;
        pushAction_({
...
function slaStatusText_(row, headerMap, etapaIdx) {
    const estado = normalizeEstado_(row && row[COL_ESTADO - 1]);
    if (estado === 'Cerrado' || estado === 'Anulado') return '-';
    ...
    for (let i = 0; i < rows.length; i++) {
        ...
        const sendReminder = (accion, to, cc, subject) => {
            ...
            const vars = buildMailVars_({
                ...
                monto: rowMontoCobro_(row, map),
                pdfUrl: resolveCobroPdfUrl_(row, map),
                actorEmail: 'sistema'
            }, {
                accion: accion,
                vence: dueTxt,
            ...
            fallbackLines.push(
                'Vence: ' + dueTxt,
                'PDF: ' + resolveCobroPdfUrl_(row, map)
            );
...
function buildGestionResult_(filtros, actorEmail, opts) {
    ...
    rows.forEach(r => {
        ...
        const incidencia = resolveCobroIncidencia_(id, itemsJson);
        const prog = progresoEtapa_(etapaVal);
        const pdfUrl = resolveCobroPdfUrl_(r, map);
...
function generarZipPdfsGestion(ids, actorEmail, actorCtx) {
    ...
    for (let i = 0; i < arr.length; i++) {
        ...
        const pdfUrl = resolveCobroPdfUrl_(row, map);
...
function setCobroObservado(id, motivo, areaDestino, actorEmail, actorCtx) {
    ...
    try {
        ...
        createStageNotifications_({
            ...
            monto: rowMontoCobro_(rowAfter, map)
        });
    } catch (e) { }
