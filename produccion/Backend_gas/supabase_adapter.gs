...
function invalidateDataStoreCache_(sheetName) {
    if (typeof invalidateCobroDetalleSubtotalMap_ === 'function') {
        invalidateCobroDetalleSubtotalMap_()
    }
    if (typeof invalidateCobroDetalleIncidenciaMap_ === 'function') {
        invalidateCobroDetalleIncidenciaMap_()
    }
            ...
            { header: 'Licencia', field: 'licencia', type: 'text' },
            {
                ...
                read: function (record) {
                    return String(
                        record && (
                            record.pdf_url ||
                            record.boleta_firmada_url ||
                            record.firma_boleta_url ||
                            ''
                        ) || ''
                    ).trim();
                }
